...
public class JTS02_DemoCameraController : MonoBehaviour {
	...
	void Start(){
		...
		zoomLevel = 0.5682f;
		zoomVal = -70.45f;
		slotPos = SlotObjects[currentSlot].position.z;
		slotPosX = SlotObjects[currentSlot].position.x;
		slotPosY = SlotObjects[currentSlot].position.y;
	...
	void LateUpdate () {
		...
		if (pivotObject != null && cameraObject != null){
			...
			vAxis = Input.GetAxis("Mouse Y") * rotationSpeed;
			hAxis = Input.GetAxis("Mouse X") * rotationSpeed;
			...
			//PIVOT MOUSE ----------------------------------------------------
			if (keydown0){
				if (pivotObject.eulerAngles.x + vAxis <0f) vAxis = 0f;
				if (pivotObject.eulerAngles.x + vAxis >79f) vAxis = 0f;
				...
				if (pivotObject.eulerAngles.x < 0f){
					pivotObject.eulerAngles = new Vector3(2f, pivotObject.eulerAngles.y, pivotObject.eulerAngles.z);
				}
			...
			slotTimer += Time.deltaTime * moveSpeed;
			slotPos = Mathf.SmoothStep(slotPos,SlotObjects[currentSlot].position.z,slotTimer);
			slotPosX = Mathf.SmoothStep(slotPosX,SlotObjects[currentSlot].position.x,slotTimer);
			slotPosY = Mathf.SmoothStep(slotPosY,SlotObjects[currentSlot].position.y,slotTimer);
			...
			pivotObject.localPosition = new Vector3(
				...
				Mathf.Lerp(slotPosY - 0.3f, slotPosY, zoomLevel),
				Mathf.Lerp(slotPos+0.189f, slotPos, zoomLevel)
				);
